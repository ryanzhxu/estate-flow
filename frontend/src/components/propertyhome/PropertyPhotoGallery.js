...
function PropertyPhotoGallery({photos}) {
    ...
    return (
        <div className="photo-gallery">
            <div className="photo-arrow" id="left-arrow" onClick={goToPrev}>❰</div>
            <div className="photo-arrow" id="right-arrow" onClick={goToNext}>❱</div>
            <div className="property-photo" style={{backgroundImage: `url(${photos[index]})`}} ></div>
            <div className="nav-dots">
                {photos.map((photo, i) => <div className={`nav-dot ${index === i ? "active" : ""}`} onClick={() => setIndex(i)} key={i}>●</div> )}
            </div>
        </div>
    )
}
